...
# READ: Query the data
names: List[str] = session.query(Record.data["name"]).all()
print("Names in JSON data:", names)

# UPDATE: Update a record
try:
    record_to_update: Record = session.query(Record).filter(Record.id == 1).one()
...
try:
    record_to_delete: Record = session.query(Record).filter(Record.id == 2).one()
...
# Verify changes
results: List[Record] = session.query(Record).all()
